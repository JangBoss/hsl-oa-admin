								...
								<tr>
									...
									<c:if test="${PowerButtom.edit == 1 }">
										...
										<c:forEach items="${buttonlist}" var="fhbutton" varStatus="vsb">
											<th class='center'>${fhbutton.btnName }</th>
										</c:forEach>
									</c:if>
								</tr>
								</thead>
										...
										<c:if test="${PowerButtom.edit == 1 }">
											...
											<c:forEach items="${buttonlist}" var="fhbutton" varStatus="vsb">
												...
												<c:forEach items="${roleFhbuttonlist}" var="varRb" varStatus="vsRb">
													<c:if test="${var.roleId == varRb.roleId && fhbutton.buttonId == varRb.buttonId }">
														<c:set value="1" var="rbvalue"></c:set>
													</c:if>
												</c:forEach>
												<td class='center' style="height: 20px;">
													<label>
														<input name="switch-field-1" onclick="upRb('${var.roleId}','${fhbutton.buttonId}')" class="ace ace-switch ace-switch-3" type="checkbox" <c:if test="${rbvalue == 1 }">checked="checked"</c:if> >
														<span class="lbl"></span>
													</label>
												</td>
